...
for global_step in range(trainer_conf['total_step']):
    ...
    for (src_img, src_label),(tgt_img) in zip(src_loader,tgt_loader):
        ...
        #
        # # Records
        # if trainer_conf['save_log'] and (count % trainer_conf['verbose_step'] == 0):
        ...
        #
        #     print(f"VAE Loss: {adv_loss.data}, Recon Loss: {recon_loss}, tMSE: {mse.data}, KL: {kl.data}")
        #
        # # update lambda
        # for k in loss_lambda.keys():
        #     if loss_lambda[k]['inc'] * loss_lambda[k]['cur'] < loss_lambda[k]['inc'] * loss_lambda[k]['final']:
        ...
        #     'save_best_only']:
        #     print("Saving model")
        #     torch.save(vae, model_path.format(global_step) + 'vae.pt')
        #     torch.save(d_digit, model_path.format(global_step) + 'dnet.pt')
        #
        # ### Show result
        # if global_step % trainer_conf['plot_step'] == 0:
        #     vae.eval()
        ...
        #
        #         torch.save(vae, model_path + 'vae.pt')
        #         torch.save(d_digit, model_path + 'dnet.pt')
        #
        #     if trainer_conf['save_log']:
        #         writer.add_scalars('Accuracy', {'source': source_acc}, global_step)
        #     if trainer_conf['save_fig']:
        #         writer.add_image('interpolate', torch.FloatTensor(np.transpose(fig1, (2, 0, 1))), global_step)
        #
        #     vae.train()
