...
def parse_args():
    ...
    parser.add_argument('--work_dir', help='the dir to save logs and models', default='models/pascal/')
    parser.add_argument(
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for i_batch, ((src), (tgt)) in enumerate(zip(dataloader_src[0], dataloader_tgt)):
            ...
            losses = s2anet.train_step(src_img, src_meta, src_gtbboxes, src_gtlabels, optimizer_s2anet)
            # losses, src_fea = s2anet(src_img, src_meta, src_gtbboxes, src_gtlabels)
            # losses.backward()
            # [losses[y].backward() for y in losses.keys()]

            loss_dict_reduced = reduce_loss_dict(losses)
            ...
            # # tgt_fea = s2anet(tgt_img, src_meta, src_gtbboxes, src_gtlabels, source = False)
        ...
        meta = {}
        meta['epoch'] = epoch
        meta['CLASSES'] = s2anet.CLASSES

        if epoch % 10 == 0 or epoch == num_epochs:
            logger.info("Saving model")
            filename1 = os.path.join(args.work_dir, "model_det_{:03d}.pth".format(epoch))
            filename2 = os.path.join(args.work_dir, "model_uda_{:03d}.pth".format(epoch))
            ...
            torch.save({'meta': meta, 'state_dict': s2anet.state_dict(), 'optimizer': optimizer_s2anet.state_dict()}, filename1)
